...
Lancer::Lancer(const Vector2D& pos)
{
	position = pos;
	timer = 0.0f;
	invincibilityTimeCounter = 0;
	enemyNumber = EnemyNumber::LANCER;
...
void Lancer::Move()
{
	...
	if (isTrace)
	{
		position.x += dir * abs(Velocity.x) * dt * 5.0f;
	}
	else
	{
...
void Lancer::Attack()
{
	...
	if (timer > enemyStatus.coolTime)
	{
		...
		{
			new Slash(Apos, SlashNumber::LANCER, (distance.x < 0), ObjectTag::ENEMY);
			SetTimer(0);
		}
	}
}
