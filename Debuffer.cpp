...
Debuffer::Debuffer(const Vector2D& pos)
{
	position = pos;
	timer = 0.0f;
	invincibilityTimeCounter = 0;
	enemyNumber = EnemyNumber::DEBUFFER;
...
void Debuffer::Attack()
{
	...
	if (timer > enemyStatus.coolTime)
	{
		if (Math2D::Length(distance) <= enemyStatus.attackDistance)
		{
			...
			new Bullet(Apos, BulletNumber::DEBUFFER, dir, ObjectTag::ENEMY);
			SetTimer(0);
		}
	}
}
